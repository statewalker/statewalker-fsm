...
export const ruleDefinitions: RuleDefinition[] = [
  ...
  },
  {
    ...
    severity: "error",
    constraint:
      "Transitions between sibling states MUST be declared ONLY in the parent's `transitions[]`, never inside a child state. Not applied to a state that enters, as its own initial sub-state, a key it does not define: that state is instantiating a shared definition, and its transitions describe that borrowed sub-machine rather than sibling routing",
  },
  {
    category: "structural",
    ruleId: "S4",
  ...
  },
  {
    ...
    severity: "error",
    constraint:
      "Leaf states MUST declare an `events` field. A state is a leaf only when it drives nothing — no `states[]` AND no `transitions[]`. A state declaring transitions is composite even with no `states[]` of its own, since the sub-states it drives resolve to definitions further up the chain",
  },
  {
    category: "structural",
    ruleId: "S9",
