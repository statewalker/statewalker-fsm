...
Implements **24 rules** across three tiers — lexical, structural, and semantic — to catch configuration errors, suspicious patterns, and advisory hints before runtime. Zero runtime dependencies.
...
const config = {
  ...
  states: [
    { key: "Off", events: ["toggle"] },
    { key: "On", events: ["toggle"] },
  ],
};
...
console.log(result.issues);   // [] — all issues (errors + warnings + info)
```
...
- **Info** — advisory hints for human review (complexity, convergent transitions, missing event declarations)

Only errors affect the `valid` flag. Warnings and info are reported but don't make the config invalid.
...
type FsmStateConfig = {
  ...
  events?: string[];                        // events this state can emit
  name?: string;
  description?: string;
  outcome?: string;
  actors?: string[];
...
| L7 | warning | Items in `events[]` should be camelCase |
...
| S3 | info | Sibling transitions should be at parent level (advisory — not enforceable statically) |
...
### Tier 3 — Semantic (M1–M7)

Consistency and completeness checks that require cross-referencing states, events, and transitions.
...
| M4 | info | Reports convergent transitions (same source, different events, same target) for human review |
